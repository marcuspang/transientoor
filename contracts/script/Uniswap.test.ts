...
describe("Uniswap V2 and V3 Tests:", () => {
  ...
  beforeEach(async () => {
    ...
    router = (await deployUniversalRouter(permit2)).connect(
      bob
    ) as UniversalRouter;
    ...
    await daiContract
      .connect(alice)
      .transfer(bob.address, expandTo18DecimalsBN(100000));
    await wethContract
      .connect(alice)
      .transfer(bob.address, expandTo18DecimalsBN(100));
    await usdcContract
      .connect(alice)
      .transfer(bob.address, expandTo6DecimalsBN(100000));
  ...
  describe("Trade on Uniswap with Permit2, giving approval every time", () => {
    describe("ERC20 --> ERC20", () => {
      ...
      it("V2 exactIn, permiting the exact amount", async () => {
        ...
        expect(wethBalanceAfter.sub(wethBalanceBefore)).to.be.gte(
          minAmountOutWETH
        );
      ...
      it("V2 exactIn, swapping more than max_uint160 should revert", async () => {
        ...
        const testCustomErrors = await (
          await ethers.getContractFactory("TestCustomErrors")
        ).deploy();
        await expect(executeRouter(planner)).to.be.revertedWithCustomError(
          testCustomErrors,
          "UnsafeCast"
        );
      });
  ...
  describe("Trade on UniswapV2", () => {
    ...
    describe("ERC20 --> ERC20", () => {
      ...
      it("exactIn trade, where an output fee is taken", async () => {
        ...
        const wethBalanceBeforeAlice = await wethContract.balanceOf(
          alice.address
        );
        const wethBalanceBeforeBob = await wethContract.balanceOf(bob.address);

        await router["execute(bytes,bytes[],uint256)"](
          commands,
          inputs,
          DEADLINE
        );

        const wethBalanceAfterAlice = await wethContract.balanceOf(
          alice.address
        );
        ...
        expect(
          aliceFee.add(bobEarnings).mul(ONE_PERCENT_BIPS).div(10_000)
        ).to.eq(aliceFee);
      });
    ...
    describe("ETH --> ERC20", () => {
      it("completes a V2 exactIn swap", async () => {
        ...
        const { daiBalanceBefore, daiBalanceAfter, v2SwapEventArgs } =
      ...
      it("completes a V2 exactOut swap", async () => {
        ...
        const { amount0Out: daiTraded, amount1In: wethTraded } =
          v2SwapEventArgs!;
        ...
        expect(ethBalanceBefore.sub(ethBalanceAfter)).to.eq(
          wethTraded.add(gasSpent)
        );
      });
    });
  });
  ...
  describe("Trade on UniswapV3", () => {
    ...
    describe("ERC20 --> ERC20", () => {
      it("completes a V3 exactIn swap", async () => {
        ...
        const { wethBalanceBefore, wethBalanceAfter, v3SwapEventArgs } =
          await executeRouter(planner);
    ...
    describe("ERC20 --> ETH", () => {
      it("completes a V3 exactIn swap", async () => {
        ...
        const { ethBalanceBefore, ethBalanceAfter, v3SwapEventArgs, gasSpent } =
          await executeRouter(planner);
        const { amount1: wethTraded } = v3SwapEventArgs!;

        expect(ethBalanceAfter.sub(ethBalanceBefore)).to.be.gte(
          amountOutMin.sub(gasSpent)
        );
        expect(ethBalanceAfter.sub(ethBalanceBefore)).to.eq(
          wethTraded.mul(-1).sub(gasSpent)
        );
      });
      ...
      it("completes a V3 exactOut swap", async () => {
        ...
        const { ethBalanceBefore, ethBalanceAfter, gasSpent } =
          await executeRouter(planner);
  ...
  describe("Mixing V2 and V3", () => {
    ...
    describe("Interleaving routes", () => {
      it("V3, then V2", async () => {
        ...
        const { wethBalanceBefore, wethBalanceAfter, v2SwapEventArgs } =
          await executeRouter(planner);
        const { amount1Out: wethTraded } = v2SwapEventArgs!;
    ...
    describe("Split routes", () => {
      it("ERC20 --> ERC20 split V2 and V2 different routes, each two hop, with explicit permit transfer from", async () => {
        ...
        planner.addCommand(CommandType.PERMIT2_TRANSFER_FROM, [
          DAI.address,
          Pair.getAddress(DAI, USDC),
          v2AmountIn1,
        ]);
        planner.addCommand(CommandType.PERMIT2_TRANSFER_FROM, [
          DAI.address,
          Pair.getAddress(DAI, USDT),
          v2AmountIn2,
        ]);
        ...
        expect(wethBalanceAfter.sub(wethBalanceBefore)).to.be.gte(
          minAmountOut1.add(minAmountOut2)
        );
      });
      ...
      it("ERC20 --> ETH split V2 and V3, one hop", async () => {
        ...
        expect(ethBalanceAfter.sub(ethBalanceBefore)).to.eq(
          wethOutV2.add(wethOutV3).sub(gasSpent)
        );
      });
      ...
      describe("Batch reverts", () => {
        ...
        it("2 sub-plans, neither fails", async () => {
          ...
          // aggregate slippage check
          subplan.addCommand(CommandType.SWEEP, [
            WETH.address,
            MSG_SENDER,
            planOneWethMinOut,
          ]);
          ...
          expect(daiBalanceBefore.sub(daiBalanceAfter)).to.eq(
            planOneV2AmountIn.add(planOneV3AmountIn)
          );
          expect(usdcBalanceBefore.sub(usdcBalanceAfter)).to.eq(
            planTwoV3AmountIn
          );
        });
  ...
  async function executeRouter(
    ...
    const wethBalanceBefore: BigNumber = await wethContract.balanceOf(
      bob.address
    );
    ...
    const v2SwapEventArgs = parseEvents(V2_EVENTS, receipt)[0]
      ?.args as unknown as V2SwapEventArgs;
    const v3SwapEventArgs = parseEvents(V3_EVENTS, receipt)[0]
      ?.args as unknown as V3SwapEventArgs;

    const ethBalanceAfter: BigNumber = await ethers.provider.getBalance(
      bob.address
    );
